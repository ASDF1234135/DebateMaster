...
const props = defineProps({
  colorStops: { type: Array, default: () => ['#5227FF', '#7cff67', '#5227FF'] },
...
onMounted(() => {
  nextTick(() => {
    ...
    try {
      ...
      const colorStopsArray = (props.colorStops.length >= 3 ? props.colorStops : ['#5227FF', '#7cff67', '#5227FF']).map(hexToRgb)
      ...
      const update = () => {
        ...
        if (!program) return
        timeAccum += 0.01 * (props.speed || 1)
        program.uniforms.uTime.value = timeAccum * 0.1
        program.uniforms.uAmplitude.value = props.amplitude ?? 1.0
        program.uniforms.uBlend.value = props.blend ?? 0.5
        const stops = props.colorStops?.length >= 3 ? props.colorStops : ['#5227FF', '#7cff67', '#5227FF']
